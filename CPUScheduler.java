...
public class CPUScheduler extends Thread {
    ...
    public void run() {    
        if(CalcSimulation.algo.equals("FCFS")) {
            ...
            for(int i = 1; i < jobBatch.length; i++) {
...
while(!policy.isEmpty()) {
    // Wait until all previous threads have finished
    boolean anyThreadRunning = true;
    while (anyThreadRunning) {
        anyThreadRunning = false;
        for (int j = 1; j < i; j++) {
            if (myThreads[j] != null && myThreads[j].t.isAlive()) {
                anyThreadRunning = true;
                break;
            }
        }
        // Optional: add a small sleep to avoid a busy wait
        try {
            Thread.sleep(101); // Check every 100 milliseconds
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }

    // Get GUI components for the current job
    ...
    // Dequeue the job and start its computation
    ...
    // Initialize and start the computation thread for this job
